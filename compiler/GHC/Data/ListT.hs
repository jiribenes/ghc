...
fold :: (a -> m r -> m r) -> m r -> ListT m a -> m r
fold sk fk l = runListT l sk fk
...
-- | Like '(<|>)', but doesn't backtrack. E.g., when the left argument produces
-- at least one element, it will commit to the left argument and not return any
-- elements of the right argument.
(<|.>) :: Applicative f => ListT f a -> ListT f a -> ListT f a
f1 <|.> f2 = ListT $ \sk fk -> unListT f1 (\a _fk' -> sk a fk) (unListT f2 sk fk)
